...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        #print(feat.shape)
        feat2= np.reshape(feat,(feat.shape[0],np.prod(feat.shape[1:])))
        output = feat2
        #print(output.shape)
        #print(output)
        pass
...
class fc(object):
    ...
    def forward(self, feat):
        ...
        output = np.add(np.dot(self.params[self.w_name].T,feat.T),np.reshape(self.params[self.b_name],(self.params[self.b_name].shape[0],1)))
        #print(output.shape)
        output=output.T
        pass
        #############################################################################
    ...
    def backward(self, dprev):
        ...
        self.grads[self.b_name] = np.ones((1, dprev.shape[0])).dot(dprev).reshape(self.output_dim,)
...
class leaky_relu(object):
    ...
    def forward(self, feat):
        ...
        #print(feat)
        #print(output)
        feato = feat*self.negative_slope
        outputo = np.where(output==0, feato,output)
        output=outputo 
        #print(output)
        pass
        #############################################################################
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        dfeato = np.array((self.meta>=0) * 1) * dprev
        dprevo = dprev*self.negative_slope
        dfeat = np.where(dfeato==0, dprevo,dfeato)
        pass
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training == True:
            kepto = self.rng.uniform(0.0, 1.0, feat.shape) <= self.keep_prob
            kept = kepto.astype(int)
            output = feat * kept * (1.0 / self.keep_prob)
        else:
            kept = np.ones(feat.shape, dtype=np.int8)
            output = feat
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        if self.is_training:
            dfeat = dprev*self.kept * (1.0/self.keep_prob)
        else:
            dfeat = dprev 
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        #print(logit)
        labs = np.zeros(feat.shape)
        labs[range(feat.shape[0]),label]=1
        label = labs
        #print(feat.shape)
        loss = - np.sum(np.multiply(label, np.log(logit))) / feat.shape[0]
    ...
    def backward(self):
        ...
        #############################################################################
        dlogit = - (label - logit) / label.shape[0]
        pass
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
def softmax(feat):
    ...
    #############################################################################
    exps = np.exp(feat - np.amax(feat, axis=-1, keepdims=True))
